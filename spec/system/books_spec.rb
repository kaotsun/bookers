...
describe '投稿のテスト' do
  ...
  describe 'トップ画面(root_path)のテスト' do
    ...
    context '表示の確認' do
      it 'トップ画面(root_path)に一覧ページへのリンクが表示されているか' do
        expect(page).to have_link "", href: "bookers-level1.herokuapp.com/books"
      end
  ...
  describe "一覧画面のテスト" do
    before do
      visit "bookers-level1.herokuapp.com/books"
    end
    context '一覧の表示とリンクの確認' do
      ...
      it "bookのタイトルと感想を表示し、詳細・編集・削除のリンクが表示されているか" do
          ...
          Book.all.each_with_index do |book,i|
            ...
            expect(show_link[:href]).to eq "http://localhost:3000/bookers-level1.herokuapp.com/books/" + book.id.to_s
            # Editリンク
            show_link = find_all('a')[j+1]
            expect(show_link.native.inner_text).to match(/edit/i)
            expect(show_link[:href]).to eq "http://localhost:3000/bookers-level1.herokuapp.com/books/" + book.id.to_s + "/edit"
            # Destroyリンク
    ...
    context '投稿処理に関するテスト' do
      ...
      it '投稿に失敗する' do
        ...
        expect(current_path).to eq('/bookers-level1.herokuapp.com/books')
      end
      it '投稿後のリダイレクト先は正しいか' do
        fill_in 'book[title]', with: Faker::Lorem.characters(number:5)
        fill_in 'book[body]', with: Faker::Lorem.characters(number:20)
        click_button 'Create Book'
        expect(page).to have_current_path "/bookers-level1.herokuapp.com/books/"+Book.last.id.to_s
      end
    end
  ...
  describe '詳細画面のテスト' do
    before do
      visit "bookers-level1.herokuapp.com/books/"+book.id.to_s
    end
    context '表示の確認' do
    ...
    context 'リンクの遷移先の確認' do
      it 'Editの遷移先は編集画面か' do
        edit_link = find_all('a')[0]
        edit_link.click
        expect(current_path).to eq('/bookers-level1.herokuapp.com/books/' + book.id.to_s + '/edit')
      end
      it 'Backの遷移先は一覧画面か' do
        back_link = find_all('a')[1]
        back_link.click
        expect(page).to have_current_path "/bookers-level1.herokuapp.com/books"
      end
    end
  end
  describe '編集画面のテスト' do
    before do
      visit "bookers-level1.herokuapp.com/books/"+book.id.to_s+"/edit"
    end
    context '表示の確認' do
    ...
    context 'リンクの遷移先の確認' do
      it 'Showの遷移先は編集画面か' do
        show_link = find_all('a')[0]
        show_link.click
        expect(current_path).to eq('/bookers-level1.herokuapp.com/books/' + book.id.to_s)
      end
      it 'Backの遷移先は一覧画面か' do
        back_link = find_all('a')[1]
        back_link.click
